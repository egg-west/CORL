...
class TrainConfig:
    ...
    # Wandb logging
    project: str = "CORL"
    group: str = "data-quality-antmaze-medium-play-v0"
...
def train(config: TrainConfig):
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if done:
            ...
            wandb.log({"jsrl_horizon": g_horizon}, step=guide_trainer.total_it)
    ...
    for t in range(config.max_finetunestep):
        current_horizon += 1
        
        # training online policy
        online_batch = online_replay_buffer.sample(int(config.batch_size))
        batch = [torch.cat([i, j], dim=0) for (i,j) in zip(off_batch, online_batch)]
