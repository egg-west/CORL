...
SEED_LIST = [8171282, 21659348, 60113847, 35871027]
CONFIG_INDEX = 0
SEED = SEED_LIST[CONFIG_INDEX]
GUIDE_PATH_LIST = ["iql_antmaze_medium_play_tau098171282",
             "iql_antmaze_medium_play_tau0921659348",
             "iql_antmaze_medium_play_tau0935871027",
             "iql_antmaze_medium_play_tau0960113847"]
GUIDE_LOAD_PATH = GUIDE_PATH_LIST[CONFIG_INDEX]

EXPLORATION_PATH_LIST = ["iql_antmaze_medium_play_tau07_8171282",
             "iql_antmaze_medium_play_tau07_21659348",
             "iql_antmaze_medium_play_tau07_35871027",
             "iql_antmaze_medium_play_tau07_60113847"]
EXPLORATION_LOAD_PATH = EXPLORATION_PATH_LIST[CONFIG_INDEX]
...
class TrainConfig:
    ...
    #max_timesteps: int = int(3*1e5)  # Max time steps to run environment
    #max_finetunestep: int = int(3*1e5)
    ...
    load_guide_policy: str = "./models/" + GUIDE_LOAD_PATH + "/checkpoint_499999.pt"
    load_exploration_policy: str = "./models/" + EXPLORATION_LOAD_PATH + "/checkpoint_499999.pt"
    # IQL
    ...
    # Wandb logging
    project: str = "CORL"
    group: str = "OPE-antmaze-medium-play-v0"
    name: str = "OPE-" + str(SEED) + "-v0"
...
def train(config: TrainConfig):
    ...
    exp_q_network = TwinQ(state_dim, action_dim).to(config.device)
    exp_v_network = ValueFunction(state_dim).to(config.device)
    exp_actor = (
        DeterministicPolicy(state_dim, action_dim, max_action)
        if config.iql_deterministic
        else GaussianPolicy(state_dim, action_dim, max_action)
    ).to(config.device)
    exp_v_optimizer = torch.optim.Adam(exp_v_network.parameters(), lr=3e-4)
    exp_q_optimizer = torch.optim.Adam(exp_q_network.parameters(), lr=3e-4)
    exp_actor_optimizer = torch.optim.Adam(exp_actor.parameters(), lr=3e-4)
    ...
    exp_kwargs = {
        "max_action": max_action,
        "actor": exp_actor,
        "actor_optimizer": exp_actor_optimizer,
        "q_network": exp_q_network,
        "q_optimizer": exp_q_optimizer,
        "v_network": exp_v_network,
        "v_optimizer": exp_v_optimizer,
        "discount": config.discount,
        "tau": config.tau,
        "device": config.device,
        # IQL
    ...
    exploration_trainer = ImplicitQLearning(**exp_kwargs)
    ...
    #"""
    if config.load_exploration_policy != "":
        policy_file = Path(config.load_exploration_policy)
        exploration_trainer.load_state_dict(torch.load(policy_file))
        exploration_actor = exploration_trainer.actor
    #"""
    ...
    # online fine-tuning
    guide_actor.eval()
    exploration_actor.eval()
    def get_guide_horizon(step):
        total_steps = config.max_finetunestep
        longest_horizon = 1000
        return int(longest_horizon * np.exp(-5.0 * (step/total_steps)))

    def epsilon_greedy(step, initial_eps=0.9, final_eps=0.05):
        #total_steps = config.max_finetunestep
        #ends_step = 50_000
        ends_step = 400_000
        if(step > ends_step):
            return final_eps
        return initial_eps * (1-(step / ends_step))
    ...
    for t in range(int(config.max_finetunestep)):
        ...
        if done:
            ...
            wandb.log({"jsrl_horizon": g_horizon, "epsilon": eps}, step=guide_trainer.total_it+online_trainer.total_it)
        ...
        if online_replay_buffer._size > config.batch_size:
            ...
            #batch = replay_buffer.sample(int(config.batch_size*(0.75)))
