...
class TrainConfig:
    ...
    max_timesteps: int = int(5*1e5)  # Max time steps to run environment
    max_finetunestep: int = int(5*1e5)
    #max_timesteps: int = int(1e4)
    checkpoints_path: str = "./models/iql_"+str(SEED)  # Save path
    load_model: str = ""#"./models/iql_antmaze_medium_play_tau09/checkpoint_299999.pt"  # Model load file name, "" doesn't load
    # IQL
    ...
    # Wandb logging
    project: str = "CORL"
    group: str = "IQL-tau07-halfcheetah-medium-v2"
    name: str = "IQL-"+str(SEED)
...
def train(config: TrainConfig):
    ...
    for t in range(int(config.max_finetunestep)):
        ...
        if config.normalize_reward and "antmaze" in config.env:
            reward -= 1.0
        replay_buffer.add_transition(state, action, reward, done, next_state)
        state = next_state
