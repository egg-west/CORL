...
class TrainConfig:
    ...
    # LPFT
    life_period: int = int(5 * 1e5) # TODO try 1e5 and larger
    warm_up: int = int(5 * 1e3)
    horizon_decrease: float = -10.0
    # Wandb logging
    project: str = "CORL"
    group: str = "LPFTV2-halfcheetah-medium-v2"
    name: str = "JSRL-learningRateRef-"+str(SEED)
...
def train(config: TrainConfig):
    ...
    # offline training (deprecated by 0 iteration)
    evaluations = []
    final_saved_t = 0
    ...
    # do we need the old data? yes 25%
    ...
    def get_guide_horizon(step):
        ...
        # -5.0 performs for jsrl, -10.0 test local optimal for lpft
        return int(longest_horizon * np.exp(config.horizon_decrease * (step/total_steps)))
    ...
    for t in range(int(config.max_finetunestep)):
        ...
        if done:
            g_horizon = get_guide_horizon(life_step)
            wandb.log({"online_horizon": current_horizon, "guide_horizon": g_horizon}, step=500000+t)
            current_horizon = 0
            state, done = env.reset(), False

        # one graident step for online policy
        if online_replay_buffer._size > config.batch_size:# and (t%5) == 0:
            off_batch = offline_replay_buffer.sample(int(config.batch_size*(0.25)))
            ...
            wandb.log(log_dict, step=500000+t)

        # life period ends, reset policy
        if t > 0 and (t % config.life_period) == 0:
            print("Policy life ends")
            #del(online_trainer)
            del(guide_trainer)
            guide_trainer = online_trainer
            online_q_network = TwinQ(state_dim, action_dim).to(config.device)
            online_v_network = ValueFunction(state_dim).to(config.device)
            online_actor = (
                DeterministicPolicy(state_dim, action_dim, max_action)
            ...
            # remains the replay buffer # iterate replay buffer
            #del(offline_replay_buffer)

            #offline_replay_buffer = online_replay_buffer
            #online_replay_buffer = ReplayBuffer(
            #    state_dim,
            #    action_dim,
            #    config.online_buffer_size,
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            """
            wandb.log(
                {"d4rl_normalized_score": normalized_eval_score}, step=500000+t
            )
