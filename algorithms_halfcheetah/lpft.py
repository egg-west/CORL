...
class TrainConfig:
    ...
    max_finetunestep: int = int(6*1e5)
    ...
    # LPFT
    life_period: int = int(2 * 1e4) # TODO try 1e5 and larger
    warm_up: int = int(5 * 1e3)
    # Wandb logging
    project: str = "CORL"
    group: str = "LPFT-antmaze-medium-play-v0"
    name: str = "JSRL-"+str(SEED)
...
def train(config: TrainConfig):
    ...
    if config.load_guide_policy != "":
        ...
        online_trainer.load_state_dict(torch.load(policy_file))
    ...
    def get_guide_horizon(step):
        ...
        return int(longest_horizon * np.exp(-10.0 * (step/total_steps)))# -0.5
    ...
    for t in range(int(config.max_finetunestep)):
        ...
        # do we need the old policy? not! we warm up the new policy in the latest data pool
