...
class TrainConfig:
    ...
    online_buffer_size: int = 1000_000
    ...
    # JSRL
    offline_batch_proportion: float = 0.25
    online_batch_proportion: float = 0.75
    horizon_decrease: float = -10.0 # usually -5.0
    # LPFT
    life_period: int = int(2*1e5)
    # Wandb logging
    project: str = "CORL"
    group: str = "LPFTV2-halfcheetah-medium-v22"
    name: str = "fromJSRL-5.0-largeReplay-"+str(SEED)
...
def train(config: TrainConfig):
    ...
    for t in range(int(config.max_finetunestep)):
        ...
        if online_replay_buffer._size > config.batch_size:
            off_batch = replay_buffer.sample(int(config.batch_size*(config.offline_batch_proportion)))
            online_batch = online_replay_buffer.sample(int(config.batch_size*(config.online_batch_proportion)))
